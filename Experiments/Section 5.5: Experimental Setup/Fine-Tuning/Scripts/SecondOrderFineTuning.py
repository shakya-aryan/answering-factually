...
def computeRouge(references, candidates): #method to compute rouge
    rouge = Rouge()
    scores = rouge.get_scores(candidates, references, avg=True)
    return scores


def computeBleu1(prediction, label): #method to compute unigram
    prediction = prediction.replace("</s>", "") #postpreocessing
    prediction = prediction.lower().split() #split into words after converting all the letters to lower case
    label = label.lower().split() #split into words after converting all the letters to lower case
    smoothingFunction = SmoothingFunction().method1 #compute smoothing to avoid null values
    return nltk.translate.bleu_score.sentence_bleu([label], prediction, weights=(1,0,0,0), smoothing_function=smoothingFunction) #since the unigram is to be computed, wight is 1,0,0,0
...
def generate_prompt(instruction, input=None): #standard prompting made available in HuggingFace
    if input:
...
lora_alpha = 16
lora_dropout = 0.01
lora_target_modules= ["q_proj", "v_proj", "k_proj"] #target the key, query and value modules
...
def train(model,
    ...
    trainData = (data["train"].map(generateAndTokenize))
    print(trainData)
    valData = (data["validation"].shuffle().take(3000).map(generateAndTokenize))
    trainer = transformers.Trainer(
        ...
        args=transformers.TrainingArguments(
            ...
            output_dir="/Results/Adapter Second-Order Effects/",
            report_to=None,
        ), 
    )
    ...
    trainer.train()
    model.save_pretrained("/Results/Adapter Second-Order Effects/")
    return model
...
def main(prompting=True, split="validation"):
    ...
    peft_model = get_peft_model(model, config) #load with peft for fine-tuning
    ...
    if not prompting:
        learning_rate = 8e-4
        peft_model = train(peft_model,data, learning_rate=learning_rate) #fine-tune the model
    sample = data['test'].shuffle().take(2000)
    ...
    # the code below is used for evaluation of the model on the test dataset
    cumulativeScore = 0
    cumulativeScore1 = 0
    cumulativeScore2 = 0
    cumulativeScore3 = 0
